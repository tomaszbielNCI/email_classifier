...
class Vectorizer:
    ...
    def create_tfidf_vectorizer(
        ...
        vectorizer = TfidfVectorizer(
            ...
            lowercase=False,  # Assume text is already lowercase
            token_pattern=r'(?u)\b\w\w+\b'  # Minimum 2 characters
        )
        
        logging.info(f"Created TF-IDF vectorizer: max_features={max_features}, ngram_range={ngram_range}")
        return vectorizer
    ...
    def vectorize_dataframe(
        ...
        """Vectorize text columns in DataFrame"""
        ...
        for col in text_columns:
            if col not in df.columns:
                logging.warning(f"Column '{col}' does not exist")
                continue
...
if __name__ == "__main__":
    ...
    if metadata['vectorizers_used']:
        ...
        print(f"\nTop 5 features: {list(top_features.items())[:5]}")
